...
mcp_server: FastMCP = FastMCP(
    name="Agent Piya integrated MCP Server",
    instructions="""
    Connect to remote Agent Piya instance.
    Agent Piya is a general AI assistant controlling it's linux environment.
    Agent Piya can install software, manage files, execute commands, code, use internet, etc.
    Agent Piya's environment is isolated unless configured otherwise.
    """,
)
...
class ToolResponse(BaseModel):
    ...
    response: str = Field(
        description="The response from the remote Agent Piya Instance"
    )
...
class ToolError(BaseModel):
    ...
    error: str = Field(
        description="The error message from the remote Agent Piya Instance"
    )
    chat_id: str = Field(description="The id of the chat this message belongs to.")


SEND_MESSAGE_DESCRIPTION = """
Send a message to the remote Agent Piya Instance.
This tool is used to send a message to the remote Agent Piya Instance connected remotely via MCP.
"""
...
async def send_message(
    message: Annotated[
        str,
        Field(
            description="The message to send to the remote Agent Piya Instance",
            title="message",
        ),
    ],
    attachments: (
        Annotated[
            list[str],
            Field(
                description="Optional: A list of attachments (file paths or web urls) to send to the remote Agent Piya Instance with the message. Default: Empty list",
                title="attachments",
            ),
        ]
        | None
    ...
    Field(
        description="The response from the remote Agent Piya Instance", title="response"
    ),
]:
...
Finish a chat with the remote Agent Piya Instance.
This tool is used to finish a persistent chat (send_message with persistent_chat=True) with the remote Agent Piya Instance connected remotely via MCP.
If you want to continue the chat, use the send_message tool instead.
Always use this tool to finish persistent chat conversations with remote Agent Piya.
"""
