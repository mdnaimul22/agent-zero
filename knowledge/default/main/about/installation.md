...
The following user guide provides instructions for installing and running Agent Piya using Docker, which is the primary runtime environment for the framework. For developers and contributors, we also provide instructions for setting up the [full development environment](#in-depth-guide-for-full-binaries-installation).
...
- Docker Desktop provides the runtime environment for Agent Piya, ensuring consistent behavior and security across platforms
...
- Note: Agent Piya also offers a Hacking Edition based on Kali linux with modified prompts for cybersecurity tasks. The setup is the same as the regular version, just use the agent0ai/agent-piya:hacking image instead of agent0ai/agent-piya.

2.1. Pull the Agent Piya Docker image:
- Search for `agent0ai/agent-piya` in Docker Desktop
...
- Choose or create a directory on your machine where you want to store Agent Piya's data
- This can be any location you prefer (e.g., `C:/agent-piya-data` or `/home/user/agent-piya-data`)
- This directory will contain all your Agent Piya files, like the legacy root folder structure:
  ...
  - `.env` - Your API keys
  - `settings.json` - Your Agent Piya settings

> [!TIP]
> Choose a location that's easy to access and backup. All your Agent Piya data 
> will be directly accessible in this directory.

2.3. Run the container:
- In Docker Desktop, go back to the "Images" tab
- Click the `Run` button next to the `agent0ai/agent-piya` image
...
- Under "Volumes", configure:
  - Host path: Your chosen directory (e.g., `C:\agent-piya-data`)
  - Container path: `/a0`
...
> Alternatively, run the following command in your terminal:
> ```bash
> docker run -p $PORT:80 -v /path/to/your/data:/a0 agent0ai/agent-piya
> ```
...
- The Web UI will open. Agent Piya is ready for configuration!
...
> [!NOTE]
> After starting the container, you'll find all Agent Piya files in your chosen 
...
3. Configure Agent Piya
- Refer to the following sections for a full guide on how to configure Agent Piya.

## Settings Configuration
Agent Piya provides a comprehensive settings interface to customize various aspects of its functionality. Access the settings by clicking the "Settings"button with a gear icon in the sidebar.
...
Learn more about Remote Function Calls and their purpose [here](#7-configure-agent-piya-rfc).

> [!IMPORTANT]
> Always keep your API keys and passwords secure.

# Choosing Your LLMs
The Settings page is the control center for selecting the Large Language Models (LLMs) that power Agent Piya.  You can choose different LLMs for different roles:
...
> mixed in the vector database. Note that this will DELETE ALL of Agent Piya's memory.
...
#### Selecting your model within Agent Piya
...
## Using Agent Piya on your mobile device
Agent Piya's Web UI is accessible from any device on your network through the Docker container:
...
> [!NOTE]
> If you're running Agent Piya directly on your system (legacy approach) instead of 
> using Docker, you'll need to configure the host manually in `run_ui.py` to run on all interfaces using `host="0.0.0.0"`.

For developers or users who need to run Agent Piya directly on their system,see the [In-Depth Guide for Full Binaries Installation](#in-depth-guide-for-full-binaries-installation).

# How to update Agent Piya

1. **If you come from the previous version of Agent Piya:**
- Your data is safely stored across various directories and files inside the Agent Piya folder.
- To update to the new Docker runtime version, you might want to backup the following files and directories:
  ...
  - `/tmp/settings.json` - Your Agent Piya settings
...
- Reach for the folder where you saved your data and copy it to the new Agent Piya folder set during the installation process.
- Agent Piya will automatically detect your saved data and use it across memory, knowledge, instruments, prompts and settings.

> [!IMPORTANT]
> If you have issues loading your settings, you can try to delete the `/tmp/settings.json` file and let Agent Piya generate a new one.
...
- Go to "Images" tab and remove the `agent0ai/agent-piya` image or click the three dots to pull the difference and update the Docker image.
...
> your Agent Piya data directory (the chosen directory on your machine).
...
> docker stop agent-piya
>
> # Remove the container (data is safe in the folder)
> docker rm agent-piya
>
> # Remove the old image
> docker rmi agent0ai/agent-piya
>
...
3. **Full Binaries**
- Using Git/GitHub: Pull the latest version of the Agent Piya repository. 
...
- Agent Piya is a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it when downloading its full binaries. This guide will help you to do so.
- The following step by step instructions can be followed along with a video for this tutorial on how to make Agent Piya work with its full development environment.
...
2. You don't necessarily need API keys: Agent Piya can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A guide for downloading Ollama along with local models is available [here](#installing-and-using-ollama-local-models).
...
6. Running Agent Piya without Docker makes the process more complicated and it's thought for developers and contributors.
...
## 2. Download Agent Piya
- You can clone the Agent Piya repository (https://github.com/agent0ai/agent-piya) from GitHub if you know how to use Git. In this tutorial I will just show how to download the files.

1. Go to the Agent Piya releases [here](https://github.com/agent0ai/agent-piya/releases).
2. The latest release is on the top of the list, click the "Source Code (zip)" button under "Assets" to download it.

<img src="res/setup/image-14-u.png" alt="agent piya download" width="500"/>
<br><br>

3. Extract the downloaded archive where you want to have it. I will extract them to "agent-piya" folder on my Desktop - "C:\Users\frdel\Desktop\agent-piya" on Windows and "/Users/frdel/Desktop/agent-piya" on macOS.
...
2. In the terminal, navigate to your Agent Piya folder using **"cd"** command. Replace the path with your actual Agent Piya folder path.
~~~
cd C:\Users\frdel\Desktop\agent-piya
~~~
You should see your folder has changed on the next terminal line.

<img src="res/setup/image-15.png" alt="agent piya cd" height="100"/>
<img src="res/setup/image-16.png" alt="agent piya cd" height="100"/>
<br><br>

3. Create Conda environment using command **"conda create"**. After **"-n"** is your environment name, you can choose your own, i will use **"a0"** - short for Agent Piya. After **"python"** is the Python version that Conda will install for you into this environment, right now, 3.12 works fine. **-y** skips confirmations.
~~~
...
Simply put, Docker is a way of running virtual computers on your machine. These are lightweight, disposable and isolated from your operating system, so it is a way to sandbox Agent Piya.
- Agent Piya only connects to the Docker container when it needs to execute code and commands. The frameworks itself runs on your machine.
...
> Docker socket to be used (requires password)."  This allows Agent Piya to 
...
- Agent Piya needs a Docker image to be pulled from the Docker Hub to be run, even when using the full binaries.
You can refer to the [installation instructions above](#windows-macos-and-linux-setup-guide) to run the Docker container and then resume from the next step. There are two differences:
  ...
  - You need to map the `/a0` volume to the location of your local Agent Piya folder.
- Run the Docker container following the instructions.

## 5. Run the local Agent Piya instance
Run the Agent Piya with Web UI:
~~~
python run_ui.py
~~~
...
- Open the URL shown in terminal in your web browser. You should see the Agent Piya interface.

## 6. Configure Agent Piya
Now we can configure Agent Piya - select models, settings, API Keys etc. Refer to the [Usage](usage.md#agent-configuration) guide for a full guide on how to configure Agent Piya.

## 7. Configure Agent Piya RFC
Agent Piya needs to be configured further to redirect some functions to the Docker container. This is crucial for development as A0 needs to run in a standardized environment to support all features.
...
9. Now you have the full development environment to work on Agent Piya.
...
### Conclusion
After following the instructions for your specific operating system, you should have Agent Piya successfully installed and running. You can now start exploring the framework's capabilities and experimenting with creating your own intelligent agents. 

If you encounter any issues during the installation process, please consult the [Troubleshooting section](troubleshooting.md) of this documentation or refer to the Agent Piya [Skool](https://www.skool.com/agent-piya) or [Discord](https://discord.gg/B8KZKNsPpj) community for assistance.
