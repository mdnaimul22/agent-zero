# Development manual for Agent Piya
This guide will show you how to setup a local development environment for Agent Piya in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - Agent Piya runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- Agent Piya is available on GitHub [github.com/agent0ai/agent-piya](https://github.com/agent0ai/agent-piya).
- You can download the files using a browser and extract or run `git clone https://github.com/agent0ai/agent-piya` in your desired directory.

> [!NOTE]
> In my case, I used `cd ~/Desktop` and `git clone https://github.com/agent0ai/agent-piya`, so my project folder is `~/Desktop/agent-piya`.
...
- Agent Piya comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.

1. Open your IDE and open the project folder using `File > Open Folder` and select your folder, in my case `~/Desktop/agent-piya`.
...
## Step 4: Run Agent Piya in the IDE
Great work! Now you should be able to run Agent Piya from your IDE including real-time debugging.
...
After inserting my API key in settings, my Agent Piya instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of Agent Piya in Docker
...
1. Pull the docker image `agent0ai/agent-piya` from Docker Hub and run it with a web port (`80`) mapped and SSH port (`22`) mapped.
...
This is how it looks in my example: port `80` is mapped to `8880` on the host and `22` to `8822`, `/a0` folder mapped to `/Users/frdel/Desktop/agent-piya`:
...
You have successfully set up a complete Agent Piya development environment! You now have:
...
- ✅ The ability to develop, debug, and test Agent Piya features seamlessly

You're now ready to contribute to Agent Piya, create custom extensions, or modify the framework to suit your needs. Happy coding! 💻✨
...
- Navigate to your project root in the terminal and run `docker build -f DockerfileLocal -t agent-piya-local --build-arg CACHE_DATE=$(date +%Y-%m-%d:%H:%M:%S) .`
