# Agent Piya Backup/Restore Backend Specification

## Overview
This specification defines the backend implementation for Agent Piya's backup and restore functionality, providing users with the ability to backup and restore their Agent Piya configurations, data, and custom files using glob pattern-based selection. The backup functionality is implemented as a dedicated "backup" tab in the settings interface for easy access and organization.
...
- Integrates with Agent Piya's established error handling and toast notification system
...
backup_section: SettingsSection = {
    ...
    "description": "Backup and restore Agent Piya data and configurations using glob pattern-based file selection.",
    "fields": [
        {
            "id": "backup_create",
            "title": "Create Backup",
...
def _get_default_patterns(self) -> str:
    ...
    return f"""# Agent Piya Knowledge (excluding defaults)
{agent_root}/knowledge/**
!{agent_root}/knowledge/default/**

# Agent Piya Instruments (excluding defaults)
...
> **⚠️ CRITICAL FILE NOTICE**: The `{agent_root}/.env` file contains essential configuration including API keys, model settings, and runtime parameters. This file is **REQUIRED** for Agent Piya to function properly and should always be included in backups alongside `settings.json`. Without this file, restored Agent Piya instances will not have access to configured language models or external services.
...
class BackupCreate(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        patterns = input.get("patterns", "")
        include_hidden = input.get("include_hidden", False)
        backup_name = input.get("backup_name", "agent-piya-backup")
...
The BackupService leverages Agent Piya's existing file operation helpers which already support RFC (Remote Function Call) routing for development mode. This ensures seamless operation whether running in direct mode or with container isolation.
...
class BackupService:
    """Core backup and restore service for Agent Piya"""

    def __init__(self):
        self.agent_zero_version = self._get_agent_zero_version()
        self.agent_zero_root = files.get_abs_path("")  # Resolved Agent Piya root
    ...
    def _get_agent_zero_version(self) -> str:
        """Get current Agent Piya version"""
        try:
    ...
    async def create_backup(self, patterns: str, include_hidden: bool = False, backup_name: str = "agent-piya-backup") -> str:
    ...
    def create_backup_with_progress(self, patterns: str, include_hidden: bool = False, backup_name: str = "agent-piya-backup"):
...
#### Agent Piya Internal Dependencies
The backup system requires these Agent Piya helper modules:
...
#### Integration with Agent Piya Error System
The backup system integrates with Agent Piya's existing error handling infrastructure:

```python
from python.helpers.errors import format_error
from python.helpers.print_style import PrintStyle

# Follow Agent Piya's error handling patterns
try:
...
Agent Piya's existing task scheduler could be extended to support automated backups:
...
The backup system uses the same version detection method as Agent Piya's main UI:
...
- **Environment Details**: User, timezone, working directory, runtime mode, Agent Piya root path
...
- **Compatibility Data**: Agent Piya version and environment for restoration validation
...
This enhanced backend specification provides a production-ready, comprehensive backup and restore system with advanced metadata tracking, real-time progress monitoring, and intelligent file management capabilities, all while maintaining Agent Piya's architectural patterns and security standards.
...
- **Testing Verified**: BackupService initializes correctly and detects Agent Piya root paths
...
**Result:** Critical files like `.env` are now properly backed up when explicitly specified, ensuring Agent Piya configurations are preserved.
...
The Agent Piya backup system is now production-ready and battle-tested! 🚀
...
- Works seamlessly between different Agent Piya installations
...
The Agent Piya backup system now provides:
