# Usage Guide
This guide explores usage and configuration scenarios for Agent Piya. You can consider this as a reference post-installation guide.

![Utility Message with Solutions](res/memory-man.png)

## Basic Operations
Agent Piya provides several basic operations through its interface:

### Restart Framework
The Restart button allows you to quickly restart the Agent Piya framework without using the terminal:
...
### Action Buttons
Located beneath the chat input box, Agent Piya provides a set of action buttons for enhanced control and visibility:
...
### File Browser: Manage files in the Agent Piya environment
...
### File Attachments
Agent Piya supports direct file attachments in the chat interface for seamless file operations:
...
## Tool Usage
Agent Piya's power comes from its ability to use [tools](architecture.md#tools). Here's how to leverage them effectively:

- **Understand Tools:** Agent Piya includes default tools like knowledge (powered by SearXNG), code execution, and communication. Understand the capabilities of these tools and how to invoke them.

## Example of Tools Usage: Web Search and Code Execution
Let's say you want Agent Piya to perform some financial analysis tasks. Here's a possible prompt:
...
> [!NOTE]
> The first run of `code_execution_tool` may take a while as it downloads and builds the Agent Piya Docker image. Subsequent runs will be faster.

This example demonstrates how to combine multiple tools to achieve an analysis task. By mastering prompt engineering and tool usage, you can unlock the full potential of Agent Piya to solve complex problems.

## Multi-Agent Cooperation
One of Agent Piya's unique features is multi-agent cooperation.
...
* **Hierarchy:** Agent Piya uses a [hierarchical structure](architecture.md#agent-hierarchy-and-communication), with superior agents delegating tasks to subordinates.  This allows for structured problem-solving and efficient resource allocation.

![](res/physics.png)
![](res/physics-2.png)

## Prompt Engineering
Effective prompt engineering is crucial for getting the most out of Agent Piya. Here are some tips and techniques:

* **Be Clear and Specific:** Clearly state your desired outcome.  The more specific you are, the better Agent Piya can understand and fulfill your request.  Avoid vague or ambiguous language.
...
## Voice Interface
Agent Piya provides both Text-to-Speech (TTS) and Speech-to-Text (STT) capabilities for natural voice interaction:
...
### File Browser
Agent Piya provides a powerful file browser interface for managing your workspace:
...
> [!NOTE]
> The files browser allows the user to go in the Agent Piya root folder if you click the `Up` button, but the working directory of Agents will always be `/work_dir`
>
- **File Operations**:
...
> [!TIP]
> The File Browser integrates seamlessly with Agent Piya's capabilities. You can reference files directly in your conversations, and the agent can help you manage, modify, and organize your files.

## Backup & Restore
Agent Piya provides a comprehensive backup and restore system to protect your data and configurations. This feature helps you safeguard your work and migrate Agent Piya setups between different systems.
...
#### What Gets Backed Up
By default, Agent Piya backs up your most important data:
...
The restore process allows you to recover your Agent Piya setup from a previous backup:
...
* **Before System Updates**: Backup before updating Agent Piya or system components
...
* **External Storage**: Keep backup files in a safe location outside Agent Piya
...
#### System Migration
Moving Agent Piya to a new server or computer:
1. Create a complete backup on the original system
2. Install Agent Piya on the new system
...
#### Team Collaboration
Sharing Agent Piya configurations:
...
> [!TIP]
> The backup system is designed to work across different operating systems and Agent Piya installations. Your backups from a Windows system will work on Linux, and vice versa.
