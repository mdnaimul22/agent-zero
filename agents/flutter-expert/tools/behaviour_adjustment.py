...
class UpdateBehaviour(Tool):
    ...
    async def execute(self, adjustments: str = "", context: str | None = None, **kwargs):

        # stringify adjustments if needed
        if not isinstance(adjustments, str):
            adjustments = str(adjustments)

        await update_behaviour(self.agent, self.log, adjustments, context)
        return Response(
...
async def update_behaviour(agent: Agent, log_item: LogItem, adjustments: str, context: str | None = None):
