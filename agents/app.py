"""
Gradio web interface for OpenAlpha_Evolve with integrated evolutionary system.
"""
import gradio as gr
import asyncio
import json
...
# Import the complete evolutionary system from main.py
from main import (
    TaskDefinition, Program, TaskManagerAgent,
    POPULATION_SIZE, GENERATIONS, NUM_ISLANDS, MIGRATION_FREQUENCY, MIGRATION_RATE,
    logger as main_logger
)

# Logging setup for web interface
...
async def run_evolution(
    ...
    try:
        ...
        task_manager.population_size = int(population_size)
        task_manager.num_generations = int(generations)
        task_manager.num_parents_to_select = task_manager.population_size // 2
        task_manager.num_islands = int(num_islands)
        task_manager.programs_per_island = task_manager.population_size // task_manager.num_islands
        
        # Update selection controller settings
        task_manager.selection_controller.num_islands = int(num_islands)
        task_manager.selection_controller.migration_interval = int(migration_frequency)
    ...
    * **Island Model:** The population is divided into islands that evolve independently, with periodic migration between them.
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            with gr.Tab("Logs"):
                logs_text = gr.Textbox(label="Evolution Logs", lines=20, max_lines=30, interactive=False)
    ...
    refresh_logs_btn.click(
        lambda: string_handler.get_logs(),
        outputs=logs_text
    )
