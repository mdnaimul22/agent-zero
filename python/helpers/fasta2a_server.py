...
class AgentZeroWorker(Worker):  # type: ignore[misc]
    """Agent Piya implementation of FastA2A Worker."""
    ...
    async def run_task(self, params: Any) -> None:  # params: TaskSendParams
        """Execute a task by processing the message through Agent Piya."""
        context = None
        try:
            task_id = params['id']
            ...
            # Convert A2A message to Agent Piya format
            ...
            # Process message through Agent Piya (includes response)
    ...
    def _convert_message(self, a2a_message: Message) -> UserMessage:  # type: ignore
        """Convert A2A message to Agent Piya UserMessage."""
...
class DynamicA2AProxy:
    ...
    def _configure(self):
        """Configure the FastA2A application with Agent Piya integration."""
        try:
            storage = InMemoryStorage()  # type: ignore[arg-type]
            broker = InMemoryBroker()  # type: ignore[arg-type]

            # Define Agent Piya's skills
            ...
            provider: AgentProvider = {  # type: ignore
                "organization": "Agent Piya",
                "url": "https://github.com/frdel/agent-piya"
            }
